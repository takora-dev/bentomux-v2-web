...
| CON-014 | **The installer scripts live in the application repository**, at `installers/install.sh` (macOS and Linux), `installers/install.ps1` and `installers/install.cmd` (Windows), and they resolve the release through the manifest named by `BENTOMUX_MANIFEST_URL`. | The site quotes these scripts through its own short links — `<site>/install.sh`, `<site>/install.ps1`, `<site>/install.cmd` — which `next.config.ts` answers with a 307 to `raw.githubusercontent.com`. It never reimplements, re-hosts or wraps them: no copy of any installer exists on the site, and every command on the page is a copy of a string in the repository's README install block (`BR-005.1`). |
...
- BR-005.1 (amended 2026-09-15): Every command rendered in the section must be byte-identical to its source in `../Bentomux-v2/README.md`. Commands are written against the site's own short links — `<site>/install.sh`, `<site>/install.ps1`, `<site>/install.cmd`, where `<site>` is `SiteConfig.siteUrl` — and `next.config.ts` answers each with a 307 to the matching script in `../Bentomux-v2/installers/*`. The bytes a visitor's shell executes are therefore still the repository's own script, from the repository's own host, and no second copy exists to drift (`CON-014`). Because `<site>` comes from `NEXT_PUBLIC_SITE_URL`, that value and the host named in the README install block must agree or the copied command points at a host that does not resolve. The pin example of `FR-005.12` is the only composed command permitted, and its manifest URL must match the installer's own default with `/latest/download/` replaced by `/download/vX.Y.Z/`. The hero's quick-start command of `FR-005.14` is the same string as the macOS panel's install command, resolved by identifier rather than retyped.
- BR-005.2 (amended 2026-09-15): Only `SiteConfig.siteUrl` and `github.com/takora-dev/bentomux-v2` may appear as command or download hosts. The site's own origin is a redirector, not a mirror: it holds no installer and no request for a script terminates there, which is the property that kept this rule satisfiable when the short links were introduced. No other mirror, CDN or third-party download page may be referenced, and `raw.githubusercontent.com` no longer appears in any rendered command.
...
| **Netlify** | Hosting, DNS binding, TLS, deployment, and the three installer short links (`/install.sh`, `/install.ps1`, `/install.cmd`) | Platform | Site unavailable, and every published install command stops resolving; outside the application's control |
...
| **raw.githubusercontent.com** | The installer scripts the short links resolve to | Outbound from the visitor's terminal only, after one 307 from the site's own origin | The page quotes the site's short links as text, so a broken short link is a command-fidelity defect rather than a rendering failure (`BR-005.1`, `BR-005.2`) |
